...
class P115RapidRetry(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._cms_domain = ""
        self._cms_api_token = ""
        self._cms_mode = "auto_organize"
        self._cms_delay_seconds = 60
        ...
        try:
            ...
            self._cms_enabled = bool(config.get("cms_enabled", False))
            ...
            if self._cms_enabled:
                try:
                    ...
                    self._cms_enabled = False
                    self._cms_client = None
                    logger.error(f"#115秒传# CMS配置无效，已关闭CMS整理 | 代码={self._safe_code(exc)}")
    ...
    @staticmethod
    def _cms_item_key(item: Dict[str, Any]) -> str:
        return str(item.get("key", "")).strip()

    def _cms_pending_items(self) -> List[Dict[str, Any]]:
        value = self.get_data("cms_pending") or []
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return [{"component": "VForm", "content": content}], {
            ...
            "empty_cleanup_enabled": False, "cms_enabled": False, "cms_domain": "", "cms_api_token": "",
            "cms_mode": "auto_organize", "cms_delay_seconds": 60, "cookie": "",
    ...
    @classmethod
    def _page_status_group(cls, status: Any) -> str:
        value = str(status or "失败").strip()
        if value == "成功":
            return "success"
        if value in {"等待重试", "风控等待"}:
            return "waiting"
        return "failure"

    @classmethod
    def _page_status_color(cls, status: Any) -> str:
        return {
            "success": "success",
            "waiting": "warning",
            "failure": "error",
    ...
    def _page_summary(self, history: List[dict]) -> Dict[str, int]:
        ...
        for item in history:
            if not item.get("media"):
                continue
            summary["total"] += 1
            summary[self._page_status_group(item.get("status"))] += 1
        return summary
    ...
    def get_page(self) -> List[dict]:
        ...
        service_state = "监控运行中" if self._enabled else "监控未启动"
        summary_items = (
            ("总任务", summary["total"], "info"),
            ("成功", summary["success"], "success"),
            ("重试中", summary["waiting"], "warning"),
            ("失败", summary["failure"], "error"),
        )
        summary_row = {
            "component": "VRow",
            "content": [
            ...
            "content": [{
                ...
                "content": [{
                    ...
                    "props": {
                        ...
                        "variant": "tonal",
                        "density": "compact",
                        "text": f"{service_state} · 最近记录：{latest_time}",
                    },
                }],
            }],
        }
        ...
        for media, episode, attempts, status, item_time in rows:
            ...
            table_content[1]["content"].append({
                "component": "tr",
                "content": [
                    {"component": "td", "text": str(media or "")},
                    {"component": "td", "text": str(episode or "-")},
                    {"component": "td", "text": f"{max(int(attempts or 0), 0)}/{self._max_retries}"},
                    {"component": "td", "content": [{
                        "component": "VChip",
                        "props": {
                            "color": self._page_status_color(status_text),
                            "size": "small",
                            "variant": "tonal",
                        },
                        "text": status_text,
